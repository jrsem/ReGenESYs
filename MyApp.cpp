...
/**
 * @brief 2-Way Chance Scenario
 * We created this scenario to test a simple 2-way chance decide, with 20% chance of following path 1.
 * Path 1 will lead to a delay
 * Path 2 to a dispose
 * 
 */
void _build2WayChance(Model* model) {
    ...
    infos->setDescription("./models/build2WayChance.txt");
...
/**
 * @brief N-Way Chance Scenario
 * We created this scenario to test a N-way chance decide, with 20% chance of following path 1, 50% of going path 2, and 30% of going path 3.
 * Path 1 will lead to a delay
 * Path 2 to dispose 2
 * Path 3 to dispose 3
 * The difference can be seen in the terminal log
 * 
 */
void _buildNWayChance(Model* model) {
    // buildModelWithAllImplementedComponents
    ModelInfo* infos = model->getInfos();
    infos->setReplicationLength(60);
    infos->setReplicationLengthTimeUnit(Util::TimeUnit::second);
    infos->setNumberOfReplications(1);
    infos->setDescription("./models/buildNWayChance.txt");
...
/**
 * @brief 2-Way Condition Scenario
 * We created this scenario to test a simple 2-way condition decide.
 * The condition used is NQ(Queue_Machine_1) <= 2*NQ(Queue_Machine_2):
 * If true, it will go to seize1
 * If false, it will go to seize2
 * 
 */
void _build2WayCondition(Model* model) {
    // buildModelWithAllImplementedComponents
    ModelInfo* infos = model->getInfos();
    ...
    infos->setDescription("./models/build2WayCondition.txt");
...
/**
 * @brief N-Way Condition Scenario
 * We created this scenario to test a N-way condition decide.
 * To follow the path 1, leading to seize1, the condition to be true is NQ(Queue_Machine_1) < 2*NQ(Queue_Machine_2)
 * To follow the path 2, leading to seize2, the condition to be true is NQ(Queue_Machine_1) >= 2*NQ(Queue_Machine_2) && NQ(Queue_Machine_1) <= 3*NQ(Queue_Machine_2)
 * If none of these conditions are match, it goes to the path3, a dispose.
 * 
 */
void _buildNWayCondition(Model* model) {
    // buildModelWithAllImplementedComponents
    ModelInfo* infos = model->getInfos();
    infos->setAnalystName("Your name");
    infos->setProjectTitle("The title of the project");
    ...
    infos->setWarmUpPeriod(50);
    infos->setWarmUpPeriodTimeUnit(Util::TimeUnit::minute);
    infos->setDescription("./models/buildNWayCondition.txt");
    ...
    decide1->getConditions()->insert("NQ(Queue_Machine_1) < 2*NQ(Queue_Machine_2)");
    decide1->getConditions()->insert("NQ(Queue_Machine_1) >= 2*NQ(Queue_Machine_2) && NQ(Queue_Machine_1) <= 3*NQ(Queue_Machine_2)");
    components->insert(decide1);
...
void MyApp::builAndRunSimulationdModel() {
    ...
    //simulator->getModelManager()->loadModel("./models/build2WayChance.txt");
    //simulator->getModelManager()->loadModel("./models/buildNWayChance.txt");
    //simulator->getModelManager()->loadModel("./models/build2WayCondition.txt");
    //simulator->getModelManager()->loadModel("./models/buildNWayCondition.txt");
